...
- [ ] **J3/J4 变现**（「变现」= 让已建成但未启用的能力真正跑起来，非商业变现）：① ✅ **两端一致性常驻守卫**（`engine_consistency_check.py`，含自检与端到端对照）② ✅ **本地演示已开服务端持久化**（`demo-config.js` 置 `remote:true` + 熔断）③ ⏳ **fat jar 部署**：**部署包已交付**（`deploy/jar/`：`start.ps1` / `start.sh` / `README-部署.md`，2026-09-23 实测从系统临时目录启动 + `-WebRoot` 指向仓库外静态副本，`/api/health` UP、静态资源全 200、公网版零密钥）→ **唯一阻塞 = 老大提供目标机器**（IP / 登录方式 / 安全组放行端口）
