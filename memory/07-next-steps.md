...
- [ ] **J3/J4 变现**（「变现」= 让已建成但未启用的能力真正跑起来，非商业变现）：① ✅ **两端一致性常驻守卫**已完成（`engine_consistency_check.py`，含自检与端到端对照）② ✅ **本地演示已开服务端持久化**（`demo-config.js` 置 `remote:true` + 熔断，`j4_remote_down_check.py` 对照通过）③ ⏳ **仍待做**：fat jar 部署到国内可达机器（摆脱 CloudBase 中间页），详见 `part6`
