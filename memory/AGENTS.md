...
| `[GATE:dag-fail] ... 存在四标签齐全的块，但块内找不到本次改动标识` 而**你确定块已落盘** | 四个标签用了**全角冒号**（`来源：` `流向：` `结构：` `异常：`）——`dag_precheck.py` 只认半角 `来源: `/`流向: `/`结构: `/`异常: `，全角形态会让整块对判据隐形，而报因不区分「没这块」与「这块没被认出」 | 改成半角冒号即 `[GATE:dag-pass]`（本轮 19:5x 实测：同一标识，仅改冒号形态即由 rc=1 转 rc=0）；别去改判据迁就写法 |
