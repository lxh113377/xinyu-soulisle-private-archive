...
7. ✅ **对标语义补测（本轮唯一新参照数据）**：16 个参照仓根目录 `.gitattributes` 探测
   ...
   本轮做法与最强者同形，不是自创规矩。
## 三、受理面复验（本轮收口证据，取 `gh run view --json conclusion` 原文）

- push `a8927d6` ⇒ run `36234766005` `conclusion=success`，四条 job 逐项 success
  （Java 构建+词表红线 / 同步守卫+评测+密钥扫描 / 浏览器回归 / 线上↔权威源逐字节新鲜度）。
- 本机同轮复跑：`--slice 0 22` ⇒ 22/22、`--slice 22 43` ⇒ 21/21 ⇒ **43/43 rc=0 ALL-GREEN**
  （含 G14 上线后的 `repo_config` 与 `repo_config_selftest`，自证断言 36→40 条）。
- 密钥扫描侧同步：`SECRET-SCAN-PASS（跟踪文件 228 个，0 命中）` —— 分母从 225 增到 228
  正是本轮新增的三个记忆卷，说明分母在跟着仓库走，不是手抄值。
