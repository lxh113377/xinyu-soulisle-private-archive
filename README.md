...
```powershell
python _test/run_all_suites.py           # ★ 全量电池（31 套件逐条直取 rc，聚合不掩盖单项失败）
...
最近实测（2026-09-25 对标轮 r20–r27）：全量电池 **31 套件 rc 全 0**；情绪评测 **73 条 / 98.6% / 危机 6-6**（JS ↔ Java 逐项全等）；`emotion_wiring_check` 9/9（后端路径实测生效 + 不可达即熔断不伪装 + 危机未经后端）；gsap 3.15.0 升级后 `browser_check`/`lightshow`/`pixel_dual` 全绿；首屏关键路径 819,767 B（预算 858,752 B 内）；公网已重新部署并 `LIVE-SYNC-PASS`。
