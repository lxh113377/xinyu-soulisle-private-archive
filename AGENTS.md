...
- [ ] 🔴 **iCAN 提交硬截止 2026-09-30**（截止即锁团队信息）：官网报名 + 提交。
      **缺件只剩一项**：报名名单 PII（五人学号/手机号/邮箱 + 指导教师 ≤2 且非成员 + 官网登录态）**只有老大能给**。
      《应用方案》PDF ✅ 不再是风险项：`交付物/提交包/心屿SoulIsle-应用方案.pdf` **20 页**（上限 20，pypdf 实数），
      官方 9 项**已由常驻判据机器核对**：`python _test/plan_pdf_coverage_check.py` →
      `PLAN-PDF-COVERAGE-PASS: 官方九项全覆盖（9/9）…分母取自大纲`（2026-09-26 r40d 立，此前"9 项齐全"只是人眼对照）。
      演示视频 ✅ 218.5s（r34 再录，含诚实标签画面）。
- [ ] **J3/J4 变现三件** —— **原列三项均已落地，此行保留只为记着它曾是 P0**：
      ① 前端切 `/api/emotion` ✅（`src/js/chat-agent.js:189` 后端可用即以后端为准 + 气泡标「情绪:后端」）
      ② `cfg.remote` ✅（分三层：代码层判定 / 本地演示 `src/js/demo-config.js:19 remote: true` / 公网层刻意不开，见 `memory/AGENTS.md` J4 三层红线）
      ③ fat jar + Docker 两条部署路均已实测就绪 ⇒ **唯一阻塞 = 老大给目标机器**（IP/登录/安全组）。
